* улучшенная по сравнению с printzag версия программы по печати заготовок
* принимает параметры 
* отбирает опционно по периоду и по материалу

* --------------------------------------------------------
* описание парметров
* parMode - определяет какой фильтр будет применяться
* 	0 - отбор не производится
*   1 - отбор только по материалу
*   2 - отбор только по периоду
*   3 - отбор и по материалу и по периоду
* в режимах, где какие-то параметры не используются
* эти параметры могут быть произвольными
* --------------------------------------------------------

* Печать ведомости заготовок для порезки
* ===============================================
lparameters parMod,parKodm,parBeg,parEnd
local kodmold
...
do 	case
	...
	case	parMod=2
		select * from raschet;
			where between(datr,parBeg,parEnd);
			order by kod, shw, shwz, kornd ;
			into cursor cp
	case	parMod=3	
		select * from raschet;
			where between(datr,parBeg,parEnd) and kod=parKodm;	
			order by kod, shw, shwz, kornd ;
			into cursor cp
	otherwise 
		wait window 'fr_printzag: Неправильно передан параметр' 
		return
endcase 
if reccount()>0
						...
						* it_masnto
						...
						scan all
							select * from mater ;
								...
								into cursor cmat
							if reccount()>0	
								aa = cpp.ra*cpp.rb*cmat.tm*cmat.uv
						...
						* it_masisp
						...
						scan all
							select * from mater ;
								...
								into cursor cmat	
							if reccount()>0
								aa = cpp.koli * cpp.rozma * cpp.rozmb * cmat.tm * cmat.uv 
